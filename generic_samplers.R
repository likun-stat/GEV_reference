...
static.metr <- function(starting.theta,
                        likelihood.fn,
                        prior.fn,
                        # hyper.params,
                        n.updates,
  ...
  # Initialize Metropolis
  theta <- starting.theta
  likelihood <- likelihood.fn(theta, ...)
  # prior <- prior.fn(theta, hyper.params)
  prior <- prior.fn(theta)
  ...
  for (i in 2:n.updates) {
    ...
    # prior.star <- prior.fn(theta.star, hyper.params)
    prior.star <- prior.fn(theta.star)
    if (prior.star != -Inf) {
      likelihood.star <- likelihood.fn(theta.star, ...)
